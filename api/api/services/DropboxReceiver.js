	...
	newReceiverStream: function newReceiverStream (options) {
		...
		receiver__._write = function onFile (__newFile, encoding, next) {
						...
						str.on('progress', function(data) {
						    console.log('Dropbox: '+data.percentage);
			...
			// 	'parents': ( options.receiverinfo.isOlympusDropboxDir ) ?[ 'root' ]: [options.receiverinfo.dropboxFsName]// parents: [ folderId ]
			// };
			// if( (typeof options.receiverinfo.dropboxFsName != 'undefined') && (options.receiverinfo.isOlympusDropboxDir == null) ){
			// 	fileMetadata.parents = [options.receiverinfo.dropboxFsName];
			// }
			// console.log(options.receiverinfo, 'dropboxFsName dropboxFsName dropboxFsName fileMetadata', fileMetadata);
			// var media = {
			...
			// 	body: __newFile//fs.createReadStream(options.path),//fs.createReadStream('files/report.csv')
			// };
			var dbxpath 	= (options.receiverinfo.dbxpath?options.receiverinfo.dbxpath+'/':'/')+__newFile.filename;
			...
			var dbxoptions 	= {
			  ...
			  headers: { //We can define headers too
			    'Authorization': 'Bearer '+dbxToken,//'10ab820c435e4dae83ee7b2b2454812d'//'9d3f0e39c220458b83c43c85c8a7b2da'
			    'Dropbox-API-Arg': '{\"path\": \"'+dbxpath+'\"}',
			    'Content-Type': 'application/octet-stream'
			  },
			  // body: __newFile,
			  rejectUnauthorized: false
			};
			var dropboxUploadReq = https.request(dbxoptions, function optionalCallback (err, httpResponse, bodymsg) {

				if(false && err) {
					// Handle error
					console.log(err, httpResponse, bodymsg);
					...
					console.log('File Id:' , httpResponse, bodymsg);

					var node_dropbox = require('node-dropbox');
					api = node_dropbox.api(dbxToken);
					api.getMetadata( dbxpath, function(err, httpResponse, body){
						if(typeof body.error == 'undefined'){
							console.log('FILE FOUND', body);

							// __newFile.extra = file;//body;
							// __newFile.extra.fsName = file.id;//fsName;
							__newFile.extra = body;
							__newFile.extra.Code = 'dbxUpload';
			...
			// __newFile.pipe(str).pipe(dropboxUploadReq);//Unsupported content with type: image/jpeg dropbox

        ...
        upload: function (options, cb) {
            ...
            async.auto({
                ...
                updateParentDirectorySizes: ['fileModel', function (cb, r) {
                        ...
                        if(options.file.isOnDrive || options.file.isOnDropbox || options.file.isOnBox){
                            //ignor in case of drive/dropbox/box
                            cb();
                        }else{
        ...
        handleUpload: function (options, cb) {
            ...
            async.auto({
                ...
                uniqueName: function (cb) {
                    if( (typeof options.name == 'undefined') || (typeof options.parentId == 'undefined') ){
                        console.log('File Model :: handleUpload ==> ERROR in initiating UniqueNameService. ');
                        return res.send(err, 500);
                    }else{
                        // UniqueNameService.unique(File, options.name, options.parentId, cb, options.replaceId);
                        //Rishabh: Do not add version text to filename
                        var UUIDGenerator = require('node-uuid');
                        var uuid = UUIDGenerator.v1();
                        var fsName = uuid + "." + _.str.fileExtension(options.name);
...
// Save file to database
                ...
                updateParentDirectorySizes: ['fileModel', function (cb, r) {
                        sails.log.debug('updating parent directories');
                        if(options.isOnDrive || options.isOnDropbox || options.isOnBox){
                            //ignor in case of drive/dropbox/box
                            cb();
                        }else{
                ...
                coallesced: ['inheritPermissions', function (cb, result) {
                        ...
                        var apiResponse = APIService.File.mini(result.fileModel);
                        apiResponse.parent.id = options.parentId;
                        apiResponse.origFileId = options.oldFile;
                        apiResponse.origFileName = options.oldFileName;
                        SocketService.broadcast('ITEM_CREATE', parentDirRoomName, apiResponse);
                        cb(null, apiResponse);
                    }],
            }, function (err, results) {
...
// Cascade delete all permissions
                ...
                sequelize.query('SELECT * FROM version where FileId =' + my.id + ' OR parent_id =' + my.id, Account)
                        .success(function (version) {
                            ...
                            version.forEach(function (listVersions) {
                                ...
                                // console.log('Model::File::rm::'+listVersions.FileId+'::v'+FileId.version);
                            });
                        });
            });
        },*/
         ...
         * TODO: Destroy binary data in object store
         * Cascade delete all permissions
         * (Rishabh Singh Chauhan) rishabh.singh@alcanzarsoft.com
         */
        rm: function (callback) {
            ...
            var rmCallback = function(e, rs){

                if(my.isOnDrive || my.isOnDropbox || my.isOnBox){
                    //ignor in case of drive/dropbox/box
                }else{
                    Directory.updateParentDirectorySizes(parentId, function () {
                    });
...
// Cascade delete all permissions
                ...
                sequelize.query('SELECT * FROM version where FileId =' + my.id + ' OR parent_id =' + my.id, Account)
                        .success(function (version) {
                            ...
                            version.forEach(function (listVersions) {
                                // console.log(listVersions);
                                console.log('Model::File::rm::'+listVersions.FileId+'::v'+listVersions.FileId.version);
                            });
                        });
                    };

            if(my.isOnDrive || my.isOnDropbox || my.isOnBox){//If On Drive : Delete the record from database
                File.findAndDelete(my.id, function (file) {

                    rmCallback(null, file);
                    console.log(file);
        ...
        mv: function (targetDirId, callback) {
            ...
            async.auto({
                ...
                updateParentDirectorySizes: ['move', 'sourceWorkgroup', 'targetWorkgroup', function (cb, results) {
                        ...
                        if(file.isOnDrive || file.isOnDropbox || file.isOnBox){
                            //ignor in case of drive/dropbox/box
                            cb();
                        }else{

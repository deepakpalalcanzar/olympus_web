		...
		rm: function(callback) {
			...
			var rmCallback = function(e, rs) {
				...
				async.auto({
					...
					updateParentDirectorySizes: function(cb) {
						sails.log.debug('updating parent directories');
						if(x.isDriveDir || x.isOlympusDriveDir || x.isDropboxDir || x.isOlympusDropboxDir || x.isBoxDir || x.isOlympusBoxDir){//ignore drive/Dropbox/Bpx
							cb();
						}else{
							Directory.updateParentDirectorySizes(parentId, cb);
						}
					...
					console.log('DELETE DROPBOX CREDENTIALS TOO', x.uploadPathId);
					if(typeof x.uploadPathId != 'undefined'){
						SyncDbox.findAll({
	                        where: {
	                            id: x.uploadPathId
	                        }
	                    }).done(function (err, dbxaccounts) {
	                        if (err)
	                            return cb(err);

	                        // Delete all the file permissions
	                        async.forEach(dbxaccounts, function (p, cb) {
	                            p.destroy().done(cb);
	                        }, callback);
	                    });
					}else{
			...
			if(x.isDriveDir || x.isOlympusDriveDir || x.isDropboxDir || x.isOlympusDropboxDir || x.isBoxDir || x.isOlympusBoxDir){//If Olympus Drive ROOT OR On Drive : Delete the record from database
                Directory.findAndDelete(x.id, function (directory) {

                    rmCallback(null, directory);
		...
		mv: function(targetDirId, callback) {
			...
			async.auto({
				...
				updateSizes: ['move', function(cb) {
					...
					if(sourceDirId.isDriveDir || sourceDirId.isOlympusDriveDir || sourceDirId.isDropboxDir || sourceDirId.isOlympusDropboxDir || sourceDirId.isBoxDir || sourceDirId.isOlympusBoxDir){

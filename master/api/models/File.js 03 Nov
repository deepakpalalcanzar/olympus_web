		...
		upload: function (options, cb) {
			...
			async.auto({
				...
				checkQuota: ['readFile', function(cb, r) {
					if (typeof options.file.size === 'undefined') { throw new Error ('No size specified!'); }
...
// Return the number of comments
			...
			sequelize.query(sql,File).success(_.unprefix(callback)).error(function(err) {throw new Error(err);});
		},
		...
		mv: function (targetDirId,callback) {
			...
			async.auto({
				...
				sourceWorkgroup: function(cb) {
					Directory.workgroup(sourceDirId, function(workgroup) {return cb(null, workgroup);});
				},

				targetWorkgroup: function(cb) {
					Directory.workgroup(targetDirId, function(workgroup) {return cb(null, workgroup);});
				},
                ...
                updateParentDirectorySizes: ['move', 'sourceWorkgroup', 'targetWorkgroup', function(cb, results) {
                    ...
                    async.parallel([
                    	function(cb){return results.sourceWorkgroup ? results.sourceWorkgroup.recalculateSize(cb, true) : cb()},
                    	function(cb){return results.targetWorkgroup ? results.targetWorkgroup.recalculateSize(cb, true) : cb();}
                    ], cb);
                }],

...
//$('span:contains("'+removeFile+'")').parents().eq(3)
	...
	onInodeDrop: function(event, ui) {
		...
		Mast.Socket.request('/'+inodeType+'/mv', {
			...
			console.log('responseMV responseMV responseMV responseMV responseMV ', response);
			if (response.error == 'canNotUploadToDrive'){
				alert('Can not Move From Olympus To Drive Directory.');
			}else if (response.error == 'canNotDownloadFromDrive'){
				alert('Can not Move From Drive To Olympus Directory.');
			}else if (response.error == 'canNotUploadToDropbox'){
				alert('Can not Move From Olympus To Dropbox Directory.');
			}else if (response.error == 'canNotDownloadFromDropbox'){
				alert('Can not Move From Dropbox To Olympus Directory.');
			}else if (response.error == 'canNotUploadToBox'){
				alert('Can not Move From Olympus To Box Directory.');
			}else if (response.error == 'canNotDownloadFromBox'){
				alert('Can not Move From Box To Olympus Directory.');
	...
	open : function(e) {
		...
		var url;
		if(typeof this.model.attributes.isOnDrive != 'undefined' && this.model.attributes.isOnDrive){
			url = this.model.attributes.viewLink;
		}else{
			url = "/file/open/"+this.get('id')+"/"+this.get('name');
		}
	...
	'delete': function() {
        ...
        var rmCloud = false;
        //Below isOlympusDriveDir not included because deleting all files require deleting all files and folders seperately rightnow
        var isOnDrive = ( self.model.attributes.isOnDrive || self.model.attributes.isDriveDir );//self.model.attributes.isOlympusDriveDir
        var isOnDropbox 	= ( self.model.attributes.isOnDropbox || self.model.attributes.isDropboxDir );//self.model.attributes.isOlympusDriveDir
        var isOnBox 		= ( self.model.attributes.isOnBox || self.model.attributes.isBoxDir );//self.model.attributes.isOlympusDriveDir

        //if Drive then confirm what to do
        if( isOnDrive && confirm('Delete '+type+' from Google Drive also?')){//olympus drive dir
			rmCloud = true;
        }

        //if Dropbox then confirm what to do
        if( isOnDropbox && confirm('Delete '+type+' from Dropbox also?')){//olympus dropbox dir
			rmCloud = true;
        }
        if( isOnBox && confirm('Delete '+type+' from Box also?')){//olympus dropbox dir
			rmCloud = true;
        }
        //if Drive/Dropbox root Directory then tell user that only olympus list will be cleared, nothing actually will be deleted from Drive/Dropbox
        if(
        	(!self.model.attributes.isOlympusDriveDir || confirm('Remove Google Drive from Olympus?') )
        	&& (!self.model.attributes.isOlympusDropboxDir || confirm('Remove Dropbox from Olympus?') )
        	&& (!self.model.attributes.isOlympusBoxDir || confirm('Remove Box from Olympus?') )
        ){
